...
import 'geolocation_handler.dart';
...
final weatherListStreamProvider = StreamProvider<List<Weather>>((ref) {
  ...
  int cureentLocationID = ref.watch(sharedPreferencesProvider).getInt('currentCityId');
  return database.watchEntries(cureentLocationID);
});
final initWeatherListStreamProvider = StreamProvider.family<List<Weather>,int>((ref,id) {
  final database = ref.watch(moorDatabaseProvider);
...
final appLocaleStateProvider = StateNotifierProvider<AppLocaleNotifier>((ref) {
  final prefs = ref.read(sharedPreferencesProvider);
  prefs.getBool('inEN') ?? prefs.setBool('inEN', true);
  final _isInEn = prefs.getBool('inEN');
  return AppLocaleNotifier(_isInEn);
});
